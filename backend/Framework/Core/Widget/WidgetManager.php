...
class WidgetManager
{
	...
	*/
	...
	public function render($view){
		...
		return Api::app()->getObjectManager()->get('dispatcher')->error($view . ' file Doesn`t exists in '. self::$widgetName .' Widget');
	}
	/**
	 * Set Script
	...
	protected function addScript($scriptFile = false){
		if($scriptFile){
			...
			$moduleScript = Api::app()->resourceUrl . DS . 'js' . DS . 'application' . DS . 'w.' . strtolower(self::$widgetName) . '.script.js';
			if(file_exists($modulePath))
	...
	protected function addStyle($styleFile = false){
		if($styleFile){
			...
			$moduleStyle = Api::app()->resourceUrl . DS . 'css' . DS . 'application' . DS . 'w.' . strtolower(self::$widgetName) . '.style.css';
			if(file_exists($modulePath))
